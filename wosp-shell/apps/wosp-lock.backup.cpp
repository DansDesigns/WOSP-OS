/*
Compile:
g++ wosp-lock.cpp -o wosp-lock -fPIC $(pkg-config --cflags --libs Qt5Widgets Qt5Gui Qt5Core)
...
// ─────────────────────────────────────────────
// Best-effort signal hardening (cannot stop SIGKILL)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// HOME resolver (fixes lockscreen service contexts)
// ─────────────────────────────────────────────
...
public:
          ...
          slidingBack(false)
    {
		...
		//How the clock position currently works
		//Vertical order in mainLayout:

		//Top status row
		//addStretch(1) ← pushes content down
		//Clock
		//addStretch(2) ← pushes clock upward relative to bottom
		//Slide text
		//The ratio of the two stretches determines where the clock sits vertically.
		...
		QFont clockFont("Comfortaa");        // or "Roboto", "DejaVu Sans", etc.
		...
		QFont labelFont("Comfortaa");        // or "Roboto", "DejaVu Sans", etc.
...
protected:
    ...
    void paintEvent(QPaintEvent *ev) override {
        ...
        if (slideTextLabel) {
            ...
            if (sliderIconAvailable && !sliderIcon.isNull()) {
                ...
                QPixmap sliderScaled = sliderIcon.scaledToHeight(desiredHeight, Qt::SmoothTransformation);
    ...
    void resizeEvent(QResizeEvent *) override { adjustScaling(); }
...
private:
    ...
    void loadIcons() {
        QString home = realHomePath();
        QString dir = home + "/.config/qtile/images/";
...
//  - AuthPage::activateInputGrab() now grabs ONLY keyboard (NOT mouse)
//  - AuthPage::deactivateInputGrab() releases ONLY keyboard
// This makes keypad buttons receive mouse/touch events again.
// ─────────────────────────────────────────────
struct ShapeItem {
    QString shape;
    QColor color;
...
private:
    ...
    void loadConfig() {
        QString home = realHomePath();
        QString cfg = home + "/.config/Alternix/.osm_lockdata";
        QFile file(cfg);
        if (!file.exists()) {
            firstRun = true;
    ...
    void saveConfig() {
        QString home = realHomePath();
        QString path = home + "/.config/Alternix/.osm_lockdata";
        QFile f(path);
...
private:
    ...
    void fadeToAuth() {
        ...
        connect(group, &QParallelAnimationGroup::finished, this, [this, group](){
            ...
            fading = false;
            group->deleteLater();
            auth->setFocus(Qt::OtherFocusReason);
        });
